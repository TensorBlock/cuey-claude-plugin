...
For any request whose result should include generated or modified downloadable files,
include `"outputDeliverables"`. When the user explicitly requests particular outputs,
add one item per output with a stable short `id`, its exact basename in `name` when
specified, its file extension in `format` when clear, and a short `description` when
needed. Do not invent a user-specified filename. When the task should produce files
but the prompt leaves their count or final set to Cuey, send an empty array so the
Smart Merger can declare the actual deliverables. Omit `"outputDeliverables"` only
when the requested result is text-only. Examples:

```json
{"outputDeliverables":[{"id":"model","name":"Acme Forecast.xlsx"},{"id":"memo","format":"pdf","description":"IC memo"}]}
```
...
Do not send `outputFileName`, `spreadsheet`, `sourceFiles`, raw `attachments`, raw `files`, `upload`, `base64`, `dataBase64`, `contentBase64`, `bytes`, `content`, extracted file text, workbook cells, formulas, `smart_merge`, `models`, `reasoningLevel`, agent names, or merge settings. Those choices belong to Cuey backend and the local Cuey runtime.
